# Why Parquet and how the Caching was Designed
...
A left join was performed on the transactions dataset on the *TransactionID* column. This was preferred to preserve all of the transactions, regardless of whether they have a matching identity record. Doing an inner join would result in a massive training data loss, since a large portion of the transactions do not have an identity associated with it.